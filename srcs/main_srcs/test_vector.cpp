...
template < class T >
void test_insert_vector(T &vec, T cpy, std::ofstream &output)
{
	...
	output << "\n\tINSERT WITH RANGE ELEMENTS FROM OTHER VEC\n";
	output << print_vec(vec.begin(), vec.end(), "Vector before insert : \n");
	output << print_vec(cpy.begin(), cpy.end(), "Cpy to insert : \n");
	...
	output << "\n\tINSERT SPECIFIC VALUE AT SPECIFIC POS WITH SPECIFIC QUANTITY\n";
	output << print_vec(vec.begin(), vec.end(), "Vector before insert count value : \n");
...
void test_exec(T &vec, std::string output_name)
{
	...
	test_insert_vector(vec, vec, output);
...
void test_vector(void)
{
	...
	std::initializer_list<my_type> list;
